...
export function Menu({ children, isOpen, onClick, anchorElement, menuOrigin, anchorOrigin }: Menu) {
   ...
   useEffect(() => {
      ...
      if (menuH === 'center') {
         ...
         if (anchorH === 'center')
            horizontalLeft = anchorBoundingCalculated.left - menuBoundingCalculated.horizontalHalf;
         if (anchorH === 'right')
            horizontalLeft =
               anchorBoundingCalculated.left -
               (menuBoundingCalculated.horizontalHalf - anchorBoundingCalculated.horizontalHalf); // TODO: TEST
   ...
   return (
      <Portal>
         ...
         <nav className={asm.join(c.Menu, show)} onClick={handleCloseMenu}>
            <ul
